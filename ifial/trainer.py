...
from .evaluator import predict, get_eval_metric_fn, EarlyStopping, EWC
from .modeling_ifial import ifialFeatureExtractor
...
class Trainer:
    ...
    def train(self):
        ...
        best_model_path = os.path.join(self.output_dir, f'models/{dataset}_best_model.pth')
        ...
        for epoch in trange(args['num_epoch'], desc='Epoch'):
            ...
            for dataindex in range(len(self.trainloader_list)):
                for data in self.trainloader_list[dataindex]:
                    if data[0]['num_col_input_ids']==None and data[0]['x_cat_input_ids']==None and data[0]['x_bin_input_ids']==None:
                        continue
            ...
            avg_train_loss = train_loss_all / ite
            ...
            if self.test_set_list is not None:
                ...
                if eval_res < best_val_loss:
                    best_val_loss = eval_res
                    # Save the best model
                    torch.save(self.model.state_dict(), best_model_path)
                    #print(f'New best validation loss: {eval_res:.6f}. Model saved.')
                
                #print(f'epoch: {epoch}, train loss: {avg_train_loss:.4f}, lr: {self.optimizer.param_groups[0]["lr"]:.6f}, spent: {time.time()-start_time:.1f} secs')
            
        # After training is done, load the best model
        if os.path.exists(best_model_path):
            logger.info(f'Loading the best model with validation loss: {best_val_loss:.6f}')
            self.model.load_state_dict(torch.load(best_model_path))

        logger.info(f'training complete, cost {time.time()-start_time:.1f} secs.')

        # Save the plot of training and validation losses
    ...
    def train(self):
        ...
        prev_ewc = None  # To store EWC instance for knowledge retention

        num_feature_subsets = len(self.trainloader_list)
        epochs_per_subset = total_epochs // num_feature_subsets


        for dataindex in range(num_feature_subsets):
            current_trainloader = self.trainloader_list[dataindex]
            current_feature_subset = self.train_set_list[dataindex][0].columns.tolist()
            print(f'Training on feature subset {dataindex+1}/{num_feature_subsets} with {len(current_feature_subset)} features')
            
            for epoch in trange(epochs_per_subset, desc=f'Feature Subset {dataindex+1} Training'):
            #for epoch in trange(args['num_epoch'], desc='Epoch'):
                ite = 0
                train_loss_all = 0
                for data in current_trainloader:
                    # Skip if all feature inputs are None
                    if data[0]['num_col_input_ids'] is None and data[0]['x_cat_input_ids'] is None and data[0]['x_bin_input_ids'] is None:
                        continue
                    self.optimizer.zero_grad()
                    logits, loss = self.model(data[0], data[1])
                ...
                # Record train loss for the current epoch
                avg_train_loss = train_loss_all / ite if ite > 0 else 0
                train_losses.append(avg_train_loss)
                ...
                if self.test_set_list is not None:
                    ...
                    print(f'Feature Subset {dataindex+1}, Epoch: {epoch}, Test {self.args["eval_metric_name"]}: {eval_res:.6f}')
                    if epoch > 500:
                        self.early_stopping(-eval_res, self.model)
                        if self.early_stopping.early_stop:
                            print('Early stopped')
            ...
            # Update EWC after each feature subset
            #prev_ewc = EWC(self.model, current_trainloader)
    ...
    def plot_losses(self, train_losses, test_losses, epochs, plot_save_dir='./plots'):
        ...
        plot_name = f'{dataset}_{epoch}_loss_plot_{seed}_{missing}_{k}.png'
        ...
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
        plt.title(f"Training and Validation Loss for k = {k} and {missing} missing rate")
        plt.legend()
        plt.grid(True)
        
        full_dir = plot_save_dir+f'/plots/seed_{seed}/{dataset}'

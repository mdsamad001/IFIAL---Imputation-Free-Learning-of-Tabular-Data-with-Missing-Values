...
def predict(clf, 
    ...
    '''Make predictions by ifialClassifier.
    ...
    -------
    pred_all: np.array
        if ``return_loss=False``, return the predictions made by ifialClassifier.

    avg_loss: float
        if ``return_loss=True``, return the mean loss of the predictions made by ifialClassifier.
...
class EWC:
    ...
    def compute_fisher_information(self):
        ...
        self.fisher_information = {n: torch.zeros(p.shape) for n, p in self.model.named_parameters() if p.requires_grad}

        # Save the current model parameters
        self.saved_params = {n: p.clone().detach() for n, p in self.model.named_parameters() if p.requires_grad}

        # Set the model to evaluation mode
        self.model.eval()

        # Compute gradients and Fisher Information for each batch
        for data in self.dataloader:
            self.model.zero_grad()
            inputs, targets = data[0], data[1]
                        # Ensure targets are PyTorch tensors

            # Forward pass
            outputs = self.model(inputs,targets)
            logits = outputs[0] if isinstance(outputs, tuple) else outputs
            # Ensure targets are PyTorch float tensors and reshape to [batch_size, 1]
            if not isinstance(targets, torch.Tensor):
                targets = torch.tensor(targets.values, dtype=torch.float).unsqueeze(1)
            else:
                targets = targets.float().unsqueeze(1)
            
            #print('logits', logits.shape)
            #print('targets', targets.shape)
            loss = torch.nn.functional.binary_cross_entropy_with_logits(logits.cpu(), targets.cpu())
            ...
            for n, p in self.model.named_parameters():
                if p.requires_grad and p.grad is not None:
                    self.fisher_information[n] += (p.grad.cpu() ** 2) / len(self.dataloader)

    def compute_ewc_loss(self, model, lambda_value):
        # Compute the EWC regularization term
        ewc_loss = 0.0
        for n, p in model.named_parameters():
            if p.requires_grad:
                # Get the Fisher Information and saved parameters
                fisher = self.fisher_information.get(n, torch.zeros_like(p).cpu())
                param_diff = (p - self.saved_params[n]) ** 2
                ewc_loss += (fisher * param_diff.cpu()).sum()
        return lambda_value * ewc_loss

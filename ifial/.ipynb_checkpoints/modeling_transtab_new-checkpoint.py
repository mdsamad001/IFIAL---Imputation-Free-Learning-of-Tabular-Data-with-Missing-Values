...
class TransTabFeatureExtractor:
    ...
    def __call__(self, x, shuffle=False) -> Dict:
        ...
        if len(num_cols) > 0:
            x_num = x[num_cols].astype(str)  # Convert numerical columns to string
            x_mask = (~pd.isna(x_num)).astype(int)  # Create mask for non-NaN values
            x_num = x_num.fillna('')  # Replace NaN with empty string
            x_num = x_num.apply(lambda col: col.name + ' ' + col) * x_mask  # Apply mask and concatenate
            x_num_str = x_num.agg(' '.join, axis=1).values.tolist()  # Join values into string for tokenization
            x_num_ts = self.tokenizer(x_num_str, padding=True, truncation=True, add_special_tokens=False, return_tensors='pt')

            encoded_inputs['x_num_input_ids'] = x_num_ts['input_ids']
            encoded_inputs['num_att_mask'] = x_num_ts['attention_mask']

        # Process Categorical Columns
        if len(cat_cols) > 0:
            x_cat = x[cat_cols].astype(str)
            x_mask = (~pd.isna(x_cat)).astype(int)  # Create mask for non-NaN values
            x_cat = x_cat.fillna('')  # Replace NaN with empty string
            x_cat = x_cat.apply(lambda col: col.name + ' ' + col) * x_mask  # Apply mask and concatenate
            x_cat_str = x_cat.agg(' '.join, axis=1).values.tolist()  # Join values into string for tokenization
        ...
        if len(bin_cols) > 0:
            x_bin = x[bin_cols].astype(str)  # Convert binary columns to string
            x_mask = (~pd.isna(x_bin)).astype(int)  # Create mask for non-NaN values
            x_bin = x_bin.fillna('')  # Replace NaN with empty string
            x_bin = x_bin.apply(lambda col: col.name + ' ' + col) * x_mask  # Apply mask and concatenate
            x_bin_str = x_bin.agg(' '.join, axis=1).values.tolist()  # Join values into string for tokenization

...
def my_multi_head_attention_forward(
    ...
    # Modify the computation of attention weights
    if need_weights:
        _B, _Nt, E = q.shape
        q_scaled = q * math.sqrt(1.0 / float(E))

        if attn_mask is not None:
            # Compute QK^T / sqrt(d_h) + M
        ...
        # Add the transpose of the mask M^T
        if attn_mask is not None:
            attn_mask_T = attn_mask.transpose(-2, -1)
            attn_output_weights = attn_output_weights + attn_mask_T
        
        #print('attn_output_weights before', attn_output_weights)
        # Apply ReLU activation function
        attn_output_weights = F.relu(attn_output_weights)
        #print('attn_output_weights', attn_output_weights)

        # Proceed with dropout and the rest of the computations
        if dropout_p > 0.0:

...
def model_pretraining(model, train_x, train_y, 
    ...
    print(f'full train cluster ratio:', '/'.join([str(np.round(x,2)) for x in cluster_ratio]), f'Total samples={debug_y.shape[0]}')
    debug_y = valid_y.cpu().numpy();
    cluster_ratio = [(debug_y==x).sum()/debug_y.shape[0] for x in np.unique(debug_y)]
    print(f'full valid cluster ratio:', '/'.join([str(np.round(x,2)) for x in cluster_ratio]), f'Total samples={debug_y.shape[0]}')

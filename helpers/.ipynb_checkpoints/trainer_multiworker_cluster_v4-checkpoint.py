# create 3 positive pairs for cluster-replace and take average
# L(p, list_neg) = p / sum(list_neg)
# loss = L(p1, list_neg_1) + L(p2, list_neg_2) + L(p3, list_neg_3)
# loss = L/3

# only use with wc-replace and one-sided-infonce
...
def model_pretraining(model, train_x, train_y, 
    ...
    def corrupt_batch(items):
        ...
        batch2a = p2.transform(batch).float()
        batch2b = p2.transform(batch).float()
        batch2c = p2.transform(batch).float()

        return batch, batch1, batch2a, batch2b, batch2c, train_y
    ...
    for i in range(10):
        corrupted_valid_x1 = valid_preprocessor1.transform(
            valid_x).float().to(device)
        corrupted_valid_x2a = valid_preprocessor2.transform(
            valid_x).float().to(device)
        corrupted_valid_x2b = valid_preprocessor2.transform(
            valid_x).float().to(device)
        corrupted_valid_x2c = valid_preprocessor2.transform(
            valid_x).float().to(device)
    ...
    for epoch in range(1, num_of_epoch+1):
        ...
        model = model.train()
        for batch_idx, (train, train1, train2a, train2b, train2c, train_y) in enumerate(data_loader):
            ...
            train2a = train2a.to(device, non_blocking=True)
            train2b = train2b.to(device, non_blocking=True)
            train2c = train2c.to(device, non_blocking=True)
            

            assert train1.shape[1] == train2a.shape[
            ...
            with torch.autograd.set_detect_anomaly(False): # disable for some slight time improvements
                ...
                # noise in 2nd branch
                e1, z1 = model(train1)
                e2a, z2a = model(train2a)
                e2b, z2b = model(train2b)
                e2c, z2c = model(train2c)

                # print('shapes z1, z2', z1.shape, ',', z2.shape)
                # print("pretraining: %fGB"%(torch.cuda.memory_allocated(0)/1024/1024/1024))

                t_start = time()
                loss1 = pretrain_loss(z1, z2a)
                loss2 = pretrain_loss(z1, z2b)
                loss3 = pretrain_loss(z1, z2c)
        ...
        for batch_idx, (valid1, valid2a, valid2b, valid2c, valid_y) in enumerate(valid_data_loader):
            # validation data already preprocessed
            with torch.no_grad():
                e1, z1 = model(valid1)
                e2a, z2a = model(valid2a)
                e2b, z2b = model(valid2b)
                e2c, z2c = model(valid2c)
